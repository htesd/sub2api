export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。默认关闭（原样透传客户端标识），需要时再显式开启；部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。 实验性 session 容量模式保留设备收敛，每 10 分钟最多 5 个根 session；超出时同一 API key 内共享根 session，保持独立 thread。单实例状态重启清空，WS 使用 ctx_pool，暂不支持 Messages/Chat Completions。',
        codexFingerprintOff: '关闭（透传，默认）',
        codexFingerprintDevice: '仅设备',
        codexFingerprintSession: '设备+会话',
        requestPolicy: {
          "title": "Codex 请求诊断与控制",
          "enabled": "启用诊断、请求预算与过载冷却",
          "description": "按账号启用，适用于 Responses / compact / WebSocket。只记录脱敏调度信息和证明头生成状态，不保存凭据或证明 token。",
          "max_attempts": "上游尝试上限",
          "retry_window_seconds": "重试窗口（秒）",
          "cooldown_seconds": "过载冷却（秒）",
          "budgetHint": "HTTP 请求、WS 新建连接和发送分别计入预算。窗口耗尽后不再发起新尝试；已开始的响应继续完成。WS 每个新回合独立计数。",
          "identity": "客户端身份处理",
          "canonical": "使用网关规范身份（默认）",
          "preserve": "保留受支持客户端的身份（对照）",
          "identityHint": "保留模式让 UA、originator 和版本保持配套；不识别或过旧的客户端仍回退规范身份。不会透传 attestation。",
          "capacity": "session 容量满额策略",
          "inherit": "沿用现有容量设置",
          "subagent": "允许子会话挂载",
          "queue": "等待根会话容量，不新增挂载",
          "capacity_wait_seconds": "容量等待上限（秒，0 为立即返回）",
          "queueHint": "仅对容量模式生效；仍优先选择其他有容量的账号。超时返回 429，已有会话继续复用。等待队列有上限。",
          "bulkApply": "修改所选账号的 Codex 请求诊断与控制"
},
        codexFingerprintCapacity: '设备收敛 + session 容量（实验）',
