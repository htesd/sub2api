export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'When multiple users share the same OAuth account, converge device/session identifiers to account-level stable values to reduce upstream-visible device and session count. Off by default (client identifiers pass through as-is); opt in explicitly when needed. Some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements. Experimental capacity mode keeps device convergence and allows 5 root sessions per 10 minutes, then shares roots within the same API key with independent threads. Single-instance state resets on restart; WS uses ctx_pool; Messages/Chat Completions are unsupported.',
        codexFingerprintOff: 'Off (passthrough, default)',
        codexFingerprintDevice: 'Device only',
        codexFingerprintSession: 'Device + Session',
        requestPolicy: {
          "title": "Codex request diagnostics and controls",
          "enabled": "Enable diagnostics, request budgets and overload cooldown",
          "description": "Opt in per account for Responses, compact and WebSocket. Records redacted routing data and attestation generation status, never credentials or attestation tokens.",
          "max_attempts": "Upstream attempt limit",
          "retry_window_seconds": "Retry window (seconds)",
          "cooldown_seconds": "Overload cooldown (seconds)",
          "budgetHint": "HTTP requests, new WS connections and sends each count. No new attempts start after the window; active responses can finish. Each new WS turn gets its own budget.",
          "identity": "Client identity handling",
          "canonical": "Gateway canonical identity (default)",
          "preserve": "Preserve supported client identity (comparison)",
          "identityHint": "Preserves a coherent UA, originator and version. Unrecognized or old clients fall back to the canonical identity. Does not forward attestation.",
          "capacity": "Session capacity overflow policy",
          "inherit": "Keep existing capacity settings",
          "subagent": "Allow subagent attachment",
          "queue": "Wait for root capacity without new attachment",
          "capacity_wait_seconds": "Capacity wait limit (seconds; 0 returns immediately)",
          "queueHint": "Only applies to capacity mode. Other available accounts are tried first. Timeout returns 429; existing sessions remain reusable. The wait queue is bounded.",
          "bulkApply": "Change Codex request diagnostics and controls for selected accounts"
},
        codexFingerprintCapacity: 'Device + session capacity (experimental)',
